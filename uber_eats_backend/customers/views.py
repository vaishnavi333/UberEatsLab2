...
print(status)  # This should output <module 'rest_framework.status'>
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    #             # If `total_price` is provided, convert it to a Decimal and update
    #             if 'total_price' in request.data:
    #                 try:
    #                      order.total_price = Decimal(str(request.data['total_price'])).quantize(Decimal('0.01'))
    #                 except (InvalidOperation, ValueError) as e:
    #                     return Response(
    #                     {'error': 'Invalid total_price value. Must be a valid decimal number.'},
    ...
    def updateOrderStatus(self, request):
        ...
        # Validate and update the order status
            ...
            # Validate and convert total_price if provided
                # if total_price:
                #     try:
                #         order.total_price = Decimal(total_price).quantize(Decimal('0.01'))
                #     except (InvalidOperation, ValueError) as e:
                #         logger.error(f"Invalid total_price value: {total_price}")
                #     return Response(
                #         {'error': f'Invalid total_price value: {total_price}'},
                #         status=status.HTTP_400_BAD_REQUEST,
                #     )
